...
const black_keys = get('template_io_black_keys')
...
for (let j = 0; j < 13; j++) {
  ...
  set('key_range_left_' + j, 0)
  set('key_range_width_' + j, 0)
}
...
indart_Ranges.forEach((indRange, i) => {
  const indSelectedRange = allart_Ranges.filter(
    (range) => range.id === indart_Ranges[i]
  )[0]
...
Object.keys(selectedRanges).forEach((range, index) => {
  ...
  var whiteKeysOnly = selectedRanges[range].white_keys_only
  ...
  if (whiteKeysOnly) {
    for (let i = 0; i < black_keys.length; i++) {
      const index = all_notes.indexOf(black_keys[i])
      set('my_key_' + index, 0)
